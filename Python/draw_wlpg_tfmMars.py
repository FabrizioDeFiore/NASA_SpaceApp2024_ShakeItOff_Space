...
out_folder_path = osp.join(root, planet, dstype, 'wavelet_package_transform')
...
for filename in tqdm(os.listdir(in_folder_path)):
    ...
    ax1.imshow(np.log(np.abs(coefficients)+1e-60), extent=[df['rel_time(sec)'].min(), df['rel_time(sec)'].max(), 0, len(nodes)],

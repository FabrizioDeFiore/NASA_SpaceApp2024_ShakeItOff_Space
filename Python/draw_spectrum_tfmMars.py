...
out_folder_path = osp.join(root, planet, dstype, 'spectrum_analysis')
...
for filename in tqdm(os.listdir(in_folder_path)):
    ...
    ax1.imshow(np.log(magnitude_spectrum), extent=[df['rel_time(sec)'].min(), df['rel_time(sec)'].max(), 0, 0.5],

...
output_folder_path = osp.join(root, planet, dstype, 'new_wavelet_transform', subaux)
...
def plot_morlet_tfm(percentage, filename, output_path):
    ...
    red_line_position = int(len(ax1) * percentage)
    plt.axvline(x=red_line_position, color='red', linestyle='--', label='Arrival Point')
